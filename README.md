...
You can then type `Ctrl+Z` and then `bg` to send it to the background to continue downloading products. The script will automatically reconnect to NWWS if the connection is dropped. I have also provided a 'nwws2' bash script to take care of running the client using nohup. You'll need to copy the file 'contrib/etc/default/nwws2' to '/etc/default' before running it.
